...
//==================================================================
    ...
    int gapcount=-1,gapStart,gapLength,gapContigNo;
    int nStart=0;//How many gaps are there?
    long int nStartPos=0,nCount;//What are those gaps start positions?
    ...
    for(int i=0;i<noContigs;i++)//For each contig
    {
        ...
        for(long int j=0;j<contigLengths[i];j++)//For the entire contig length
        {
            ...
            {
                if(nCount > 1 && nCount < readlen && nStartPos - trimsize > 2*trimsize 
