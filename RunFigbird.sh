...
function run_bowtie(){
    ...
    if [ $iteration = 1 -a $trim -gt 0 ]; then
        g++ FlankTrim.cpp && ./a.out $gapped_genome $trim $maxreadlen $trimmed_temp_genome
        
        python reference.py $trimmed_temp_genome $trimmed_genome 60
        rm $trimmed_temp_genome
    ...
    g++ -std=c++11 -pthread FillGaps.cpp && ./a.out $gapped_genome $maxdistance $maxreadlen $count $partial_flag $unmapped $num_threads $myoutfilename $temp_file_path $gapped_reads_path $neg_ovlap $partial_readlen $trim $set_inputmeanflag $m2
...
function run_bowtie_user(){
    ...
    if [ $iteration = 1 -a $trim -gt 0 ]; then
        g++ FlankTrim.cpp && ./a.out $gapped_genome $trim $mreadlen $trimmed_temp_genome
    ...
    g++ -std=c++11 -pthread FillGaps.cpp && ./a.out $genome $maxdistance_fig $mreadlen $count $p_flag $u_flag $num_threads $myoutfname  $temp_file_path $gapped_reads_path $neg_ovlap $partial_readlen $trim $set_inputmeanflag $m2
...
trimmed_temp_genome=$temp_file_path"newgenome_temp.fa"
...
if [ $eval = 1 ]; then
    ...
    if test -f "$ref_genome_path"; then
        ...
        python reference.py $new_file $EM_dir 60
